      ...
      >
        ...
        <el-table-column prop="deployCount" sortable label="共发布">
          <template #default="{ row }">
            <el-button
              @click="$router.push(`/statistics/task-list?createUserId=${row.createUserId}`)"
              link
              type="primary"
        ...
        <el-table-column prop="checking" sortable label="待核查">
          <template #default="{ row }">
            <el-button
              @click="
                $router.push(`/statistics/task-list?createUserId=${row.createUserId}&status=40`)
              "
              link
              type="primary"
        ...
        <el-table-column prop="checkNoPass" sortable label="审核退回">
          <template #default="{ row }">
            <el-button
              @click="
                $router.push(`/statistics/task-list?createUserId=${row.createUserId}&status=32`)
              "
              link
              type="primary"
        ...
        <el-table-column prop="checkPass" sortable label="审核通过">
          <template #default="{ row }">
            <el-button
              @click="
                $router.push(`/statistics/task-list?createUserId=${row.createUserId}&status=50`)
              "
              link
              type="primary"
        ...
        <el-table-column prop="gafw" sortable label="公安服务">
          <template #default="{ row }">
            <el-button
              @click="
                $router.push(`/statistics/task-list?createUserId=${row.createUserId}&type=公安服务`)
              "
              link
              type="primary"
              >{{ row.gafw }}</el-button
        ...
        <el-table-column prop="jcjc" sortable label="基层基础">
          <template #default="{ row }">
            <el-button
              @click="
                $router.push(`/statistics/task-list?createUserId=${row.createUserId}&type=基层基础`)
              "
              link
              type="primary"
              >{{ row.jcjc }}</el-button
        ...
        <el-table-column prop="fzga" sortable label="法治公安">
          <template #default="{ row }">
            <el-button
              @click="
                $router.push(`/statistics/task-list?createUserId=${row.createUserId}&type=法治公安`)
              "
              link
              type="primary"
              >{{ row.fzga }}</el-button
        ...
        <el-table-column prop="zzgz" sortable label="政治工作">
          <template #default="{ row }">
            <el-button
              @click="
                $router.push(`/statistics/task-list?createUserId=${row.createUserId}&type=政治工作`)
              "
              link
              type="primary"
              >{{ row.zzgz }}</el-button
        ...
        <el-table-column prop="djpa" sortable label="打击破案">
          <template #default="{ row }">
            <el-button
              @click="
                $router.push(`/statistics/task-list?createUserId=${row.createUserId}&type=打击破案`)
              "
              link
              type="primary"
              >{{ row.djpa }}</el-button
  ...
  methods: {
    ...
    async getList() {
      ...
      const { code, result } = await this.$http
        .post('/api/statistics/getDeployUserCount', this.searchForm)
        .finally(() => {
          this.loading = false
        })
      if (code === 200) {
